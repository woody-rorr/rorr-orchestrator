...
작업 요청 → **플래닝 단계** (`00-planning.md`) → 사용자 승인 → **도메인 MCP 호출** (이 문서)
...
- 작업 의도가 인정되는 모든 프롬프트는 토글 ON인 MCP 전부에 순차 호출한다 — backend, web, extension 등.
...
|---|---|---|
| 신규 API/모듈/기능 | `backend_new__scaffold_api({ scope, user_message })` | piecomp/rorr-backend-api |
