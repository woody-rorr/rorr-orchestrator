...
- **마지막은 반드시 `scope: "publish"` 호출**. 이 호출 1회로 brand 생성·push·PR 생성까지 MCP가 다 한다.
  ...
  - **파일 본문은 이전 scaffold 응답의 `files[<path>]` 문자열을 1바이트도 손대지 말고 그대로 복사**. `"(see previous scaffold output)"`, `"<생략>"`, `"..."`, 요약, 압축, placeholder 문자열로 바꾸는 것 **절대 금지**. publish가 abort하고 PR 누락된다 (2026-06-05 Quiz 사고).
...
- 따라서 신규 모듈 추가 요청(예: "Quiz API 추가")에서는 `bootstrap`, `app-shell`, `database` (테이블 신규일 때 제외) scope 호출 **생략**. 곷바로 `module:<name>` 부터 시작.
